...
<html lang="en-GB">
 <head>
	...
	<meta name="description" content="An etymology of Bexhill-on-Sea's street and road names."/>
	...
	<meta property="og:description" content="An etymology of Bexhill-on-Sea's street and road names."/>
	...
	<title><xsl:value-of select="info/title"/> | Bexhill-OSM</title>
 </head>

 <body>
	<img id="header" alt="Street scenes in Bexhill-on-Sea." onclick="setHeaderImg();"/>
	<button id="button-theme" title="Dark/Light Theme">
	...
	<div id="intro">
		...
		<p class="credit">Alexis Markwick, Bexhill OpenStreetMap</p>
	</div>
	<ul id="links">
		<li><a href="#research">Requiring further research</a></li>
	...
	<div id="street-filter">
		<input id="street-filter-input" type="text" placeholder="Enter a street name or year"/><span id="street-filter-close"></span>
		<p id="street-filter-text"></p>
	</div>
	<div id="street-results"><xsl:for-each select="street-names/street"><xsl:sort select="name"/>
		<xsl:element name="p"><xsl:attribute name="class"><xsl:value-of select="normalize-space(concat('street', ' ', substring(name,1,1), ' ', @style))"/></xsl:attribute>
		<strong><xsl:value-of select="name"/> (<xsl:value-of select="date"/>)</strong><br/>
		<xsl:value-of select="desc"/></xsl:element>
	</xsl:for-each></div>
	<p id="street-results-end">END</p>
	<div id="outro">
		<h2 id="research">Requiring further research</h2>
		<p>Please email us if you can offer help with any of the following:</p>
		<p id="rsrchdesc"><xsl:for-each select="street-names/street-wip"><xsl:sort select="date"/>
			<xsl:element name="span"><xsl:value-of select="name"/> (<xsl:value-of select="date"/>)</xsl:element>
		</xsl:for-each></p>
		<h2 id="appendix">Appendix</h2>
		<p><a href="https://www.bexhillmuseum.org.uk/access-centre/research/street-directories/" target="_blank" rel="noopener">Bexhill Museum Street Directories</a> (website)</p>
		...
		<ul>
			<li>Images from Bexhill Museum collections.</li>
			...
			<li>Bexhill Census, 1841-1911.</li>
			<li>Bexhill Street Directories, 1890-1974.</li>
			<li>Borough of Bexhill Year Book, 1973-74.</li>
			...
			<li>Don Philips and Ann Vollor for their work on building surveys.</li>
			<li>Bexhill OpenStreetMap, bexhill-osm.org.uk.</li>
		</ul>
	</div>
	<button id="button-anchor" title="Return to top">
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40">
			<path d="m20.64,8.45c-0.18,-0.07 -0.37,-0.11 -0.58,-0.11l-0.01,0l-0.05,0c-0.03,0 -0.05,0 -0.07,0.01c-0.21,0 -0.39,0.03 -0.56,0.1c-0.35,0.12 -0.64,0.35 -0.82,0.63l-12.03,12.07c-0.29,0.28 -0.44,0.66 -0.44,1.07c0,0.46 0.2,0.92 0.53,1.25c0.33,0.33 0.76,0.52 1.21,0.52c0.41,0 0.8,-0.16 1.09,-0.45c0.01,-0.02 0.02,-0.02 0.04,-0.04c0.02,-0.02 -1.85,1.49 0.08,-0.09c1.93,-1.58 4.86,-3.95 8.01,-5.47l-0.04,18.01c0.85,1.01 2.1,-0.98 3.05,-0.98c0.95,0 2.15,1.77 2.96,1.01c0,-5.89 -0.06,-12.03 -0.05,-17.92l8.01,5.33c0.03,0.04 0.06,0.07 0.1,0.1l0.04,0.04c0.29,0.29 0.68,0.45 1.09,0.45c0.45,0 0.87,-0.19 1.2,-0.52c0.67,-0.66 0.72,-1.7 0.1,-2.32l-12.85,-12.7zl0,0zl0,0z"/>
			<path d="m29.36,3.03l-18.98,0c-0.44,0 -0.8,0.67 -0.8,1.48c0,0.82 0.36,1.5 0.8,1.5l18.98,0c0.44,0 0.8,-0.67 0.8,-1.5c0,-0.8 -0.36,-1.48 -0.8,-1.48"/>
		</svg>
	</button>
