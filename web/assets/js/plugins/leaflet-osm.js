...
L.OSM.DataLayer = L.FeatureGroup.extend({
	...
	addData: function (features) {
		if (!(features instanceof Array)) { features = this.buildFeatures(features); }
		for (var i = 0; i < features.length; i++) {
			var feature = features[i], layer;
			if (feature.type === "changeset") layer = L.rectangle(feature.latLngBounds, this.options.styles.area);
			else if (feature.type === "node") layer = L.circleMarker(feature.latLng, this.options.styles.node);
			else {
	...
	buildFeatures: function (xml) {
		...
		for (var node_id in nodes) {
			...
			if (this.interestingNode(node, ways, relations)) features.push(node);
		}
	...
	interestingNode: function (node, ways, relations) {
		...
		for (var key in node.tags) {
			if (this.options.uninterestingTags.indexOf(key) < 0) return true;
		}
		return false;
	}
});
...
L.Util.extend(L.OSM, {
	...
	getWays: function (xml, nodes) {
		...
		for (var i = 0; i < ways.length; i++) {
			...
			for (var j = 0; j < nds.length; j++) { way_object.nodes[j] = nodes[nds[j].getAttribute("ref")]; }
	...
	getTags: function (xml) {
		...
		for (var j = 0; j < tags.length; j++) { result[tags[j].getAttribute("k")] = tags[j].getAttribute("v"); }
		return result;
	}
});
